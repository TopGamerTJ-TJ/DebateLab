...
import { Brain, Loader2, Eye, EyeOff, ChevronRight, RotateCcw, Sparkles, ListChecks, Layers } from "lucide-react";
...
3. flashcards: 5-10 active-recall flashcards (front = a prompt/question, back = the answer) covering the most important facts, arguments, and rebuttals.
...
"""${source}"""`,
  ...
  const reset = () => { setResult(null); setSource(""); setTitle(""); };
    ...
    <AnimatedPage>
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        <div className="bg-gradient-to-br from-rose-600 to-pink-700 rounded-3xl p-8 mb-6 text-white shadow-lg">
          <div className="flex items-center gap-2 mb-2 text-rose-100 text-sm font-medium"><Brain className="w-4 h-4" /> Memory Assist</div>
        ...
        {!result ? (
          ...
          <>
            ...
            <Button variant="outline" onClick={reset} className="w-full gap-2 mt-6"><RotateCcw className="w-4 h-4" /> New Material</Button>
          </>
        )}
      </div>
    </AnimatedPage>
