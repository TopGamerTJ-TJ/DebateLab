...
const FORMATS = [
  { name: "Parliamentary", tag: "Parli", desc: "British Parliamentary, American Parliamentary, and MSPDP formats with government/opposition roles, POIs, and prep time management.", tools: ["AI Contentions", "Practice Rounds", "Flow Sheet", "Case Vault"], dot: "bg-blue-400" },
  { name: "Public Forum", tag: "PF", desc: "NSDA Public Forum with crossfire prep, weighing mechanisms, summary/final focus strategy, and evidence pairing for both sides.", tools: ["Contentions", "Crossfire Prep", "Evidence", "Flow Sheet"], dot: "bg-violet-400" },
  { name: "Model UN", tag: "MUN", desc: "Complete MUN toolkit — position papers, draft resolutions, working papers, bloc strategy memos, and speech scripts for any committee.", tools: ["Position Papers", "Resolutions", "Country Profiles", "Speeches"], dot: "bg-teal-400" },
  { name: "Model Congress", tag: "Congress", desc: "Draft bills and resolutions, write authorship and floor debate speeches, and prepare legislative strategy for any chamber.", tools: ["Bill Drafting", "Speeches", "Floor Strategy", "Committee Prep"], dot: "bg-amber-400" },
];
...
export default function LandingPreview() {
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-white font-body">
      ...
      {/* ── Navbar ── */}
      <nav className="sticky top-0 z-50 bg-[#111111]/95 backdrop-blur border-b border-white/[0.06]">
        <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-14">
          ...
          <div className="flex items-center gap-2">
            <Link to="/login">
              <button className="px-4 py-1.5 text-sm text-white/60 hover:text-white border border-white/15 rounded-lg transition-all hover:border-white/30">
                Sign In
              </button>
            </Link>
            <Link to="/register">
      ...
      {activeTab === "home" && (
        <div className="flex flex-col items-center text-center px-6">
          ...
          <div className="flex items-center gap-3 mb-16">
            ...
            <Link to="/login">
              <button className="px-6 py-3 text-sm text-white/60 border border-white/15 rounded-xl hover:text-white hover:border-white/30 transition-all">
                Sign In
              </button>
            </Link>
          </div>
        ...
        <div className="max-w-3xl mx-auto px-6 pt-6 pb-20">
          ...
          <div className="bg-[#1c1c1e] border border-white/[0.07] rounded-2xl p-12 text-center mb-8">
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <Link to="/login">
                <button className="px-6 py-2.5 text-sm text-white/60 border border-white/15 rounded-xl hover:text-white hover:border-white/30 transition-all">
                  Sign In
                </button>
              </Link>
            </div>
          ...
          <div className="flex gap-3 mb-8">
            <div className="flex-1 flex items-center gap-2 bg-[#1c1c1e] border border-white/[0.07] rounded-xl px-4">
          ...
          <div>
            ...
            <div className="bg-[#1c1c1e] border border-white/[0.07] rounded-2xl p-12 text-center">
              ...
              <p className="text-white/25 text-sm">Trending topics will appear as debaters practice. Check back soon!</p>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === "formats" && (
        <div className="max-w-3xl mx-auto px-6 pt-12 pb-20">
          <div className="text-center mb-12">
            ...
            <p className="text-white/35 text-base max-w-md mx-auto">Purpose-built toolkits for the four major competitive debate and speech formats.</p>
          </div>
          <div className="space-y-4">
            {FORMATS.map(f => (
              <div key={f.name} className="bg-[#1c1c1e] border border-white/[0.07] rounded-2xl p-6 hover:border-white/15 transition-all">
                <div className="flex items-center gap-3 mb-2">
                  <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${f.dot}`} />
                  <h3 className="font-bold text-white font-heading">{f.name}</h3>
                  <span className="text-xs text-white/30 bg-white/6 px-2 py-0.5 rounded-full">{f.tag}</span>
                </div>
                <p className="text-white/40 text-sm leading-relaxed mb-4 ml-5">{f.desc}</p>
                <div className="flex flex-wrap gap-2 ml-5">
                  {f.tools.map(t => (
                    <span key={t} className="text-xs text-white/50 bg-white/5 border border-white/8 px-2.5 py-1 rounded-full">{t}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      {activeTab === "about" && (
        <div className="max-w-2xl mx-auto px-6 pt-16 pb-20 text-center">
          ...
          <div className="border-t border-white/8 pt-8 text-xs text-white/20 flex items-center justify-center gap-5 flex-wrap">
      ...
      {/* Footer */}
      <div className="border-t border-white/[0.06] py-6 px-6 text-center text-xs text-white/15 flex items-center justify-center gap-5">
        <Link to="/terms" className="hover:text-white/40 transition-colors">Terms</Link>
        <Link to="/terms" className="hover:text-white/40 transition-colors">Privacy</Link>
        <a href="mailto:DebateLab@outlook.com" className="hover:text-white/40 transition-colors">Contact</a>
